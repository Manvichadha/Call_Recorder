...
// Endpoint to manually sync/pull transcription status from AssemblyAI
app.post('/api/transcribe/sync', async (req, res) => {
  try {
    ...
    // Fetch the recording from Supabase to get the assembly_id
    ...
    if (!rec.assembly_id) {
      return res.status(400).json({ error: 'This recording has no AssemblyAI ID yet. Try triggering transcription.' });
    }

    console.log(`[SYNC] Manually pulling status for AssemblyAI ID: ${rec.assembly_id}`);
    const transcript = await aai.transcripts.get(rec.assembly_id);
    ...
    if (transcript.status === 'completed') {
      ...
      // Extract highlights as action items
      let actionItems = [];
      ...
      return res.json({ status: 'analyzed', message: 'Recording successfully synced and analyzed.' });
      ...
      return res.json({ status: 'error', message: 'Transcription failed on AssemblyAI side.' });
    }

    res.json({ status: transcript.status, message: `Transcription is still: ${transcript.status}` });
